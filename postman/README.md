...
- **Posts and Photos Tests** — посты, фильтры по адресу, фотографии, CRUD

## Переменные окружения

Можно использовать готовый environment:

- файл `Local-ESM.postman_environment.json` (лежит в этой же папке).  
  Импортируйте его через **Import → Upload Files** и выберите окружение `Local ESM`.

Или создать вручную Environment в Postman с переменными:
...
| `post_id`    | (берётся из ответа «Создать пост») | ID поста для запросов |
| `photo_id`   | (берётся из ответа загрузки фото) | ID фото для удаления |
| `test_address` | `Самарская область, Самара, Московское шоссе, 77` | Адрес для фильтров |
| `Rubric_param` | `Тестовая рубрика`       | Имя тестовой рубрики         |
...
| Поиск по адресу | GET | `{{base_url}}/address/search/?q=Самарская область, Самара, Московское шоссе, 77&limit=5` | Публичный доступ |
